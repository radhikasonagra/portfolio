...
import { 
  ...
  Terminal, Globe, Box, Cloud, Braces, Smartphone, ShieldCheck,
  FileCode2, Palette, FileJson, CheckCircle2, Cpu
} from 'lucide-react';
  ...
  return (
    <section id="skills" className="py-20 bg-darker overflow-hidden relative">
      <div className="container mx-auto px-6 relative z-10">
        ...
        {/* Modified Grid Layout: Sphere takes 8/12 (67%), List takes 4/12 (33%) */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          
          {/* Left: 3D Sphere - Expanded */}
          <div className="h-[400px] md:h-[550px] flex items-center justify-center relative order-1 lg:col-span-7">
            <SkillSphere />
          </div>

          {/* Right: 2D Vertical List - Compacted */}
...
const SkillSphere: React.FC = () => {
  ...
  useEffect(() => {
    // Responsive Radius logic to prevent overlap on mobile
    const handleResize = () => {
      if (window.innerWidth < 768) {
        setRadius(150); // Smaller radius for mobile
      } else {
        setRadius(250); // Full radius for desktop
      }
    };
      ...
      {sphereSkills.map((skill, index) => {
        ...
        return (
          ...
          >
            ...
            {/* Name Text */}
            <span className="text-[9px] md:text-[10px] font-bold text-white whitespace-nowrap bg-black/50 px-1 rounded">
                {skill.name}
            </span>
          </motion.div>
        );
